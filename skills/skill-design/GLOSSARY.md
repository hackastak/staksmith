...
The domain model for what makes a skill great. A skill exists to wrangle determinism out of a stochastic system; the root virtue is **Predictability**, and every term below is a lever on it. This is the disclosed reference for [`skill-design`](SKILL.md).
...
The ordered actions the agent performs — when a skill has them, the primary tier of its content, and the part that earns its place in SKILL.md. Not every skill has steps: a skill can be all steps (`tdd-workflow`), all **reference** (a review), or both, independent of invocation. Every step ends on a **completion criterion**, clear or vague.
...
The same term appears in `writing-fragments`, aimed at the reader rather than the agent: there a leading word is the coinage a whole article hangs on, hunted during drafting. Same mechanism, different target — don't collapse the two uses.
