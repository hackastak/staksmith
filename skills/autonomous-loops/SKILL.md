...
**staksmith's built-in persistent loop.** A session-aware REPL that calls `claude -p` synchronously with full conversation history.
...
3. **Any loop + Verification** — Use staksmith's `/verify` command or `verification-loop` skill as a gate before commits.
...
| NanoClaw | staksmith | `/claw` command in this repo |
| Verification Loop | staksmith | `skills/verification-loop/` in this repo |
