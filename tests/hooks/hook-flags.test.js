...
function runTests() {
  ...
  if (test('defaults to standard for invalid value', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: 'invalid' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;

  if (test('defaults to standard for empty string', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: '' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;
  ...
  if (test('returns empty Set when env var not set', () => {
    withEnv({ STAKSMITH_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('returns empty Set for empty string', () => {
    withEnv({ STAKSMITH_DISABLED_HOOKS: '' }, () => {
  ...
  if (test('parses single hook id', () => {
    withEnv({ STAKSMITH_DISABLED_HOOKS: 'my-hook' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('parses multiple comma-separated hook ids', () => {
    withEnv({ STAKSMITH_DISABLED_HOOKS: 'hook-a,hook-b,hook-c' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('trims whitespace around hook ids', () => {
    withEnv({ STAKSMITH_DISABLED_HOOKS: ' hook-a , hook-b ' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('normalizes hook ids to lowercase', () => {
    withEnv({ STAKSMITH_DISABLED_HOOKS: 'MyHook,ANOTHER' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('returns true by default for a hook (standard profile)', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: undefined, STAKSMITH_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('returns false when hook is in disabled list', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: undefined, STAKSMITH_DISABLED_HOOKS: 'my-hook' }, () => {
  ...
  if (test('returns true when hook is not in disabled list', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: undefined, STAKSMITH_DISABLED_HOOKS: 'other-hook' }, () => {
      assert.strictEqual(isHookEnabled('my-hook'), true);
    });
  })) passed++; else failed++;

  if (test('returns false when current profile is not in allowed profiles', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: 'minimal', STAKSMITH_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('returns true when current profile is in allowed profiles', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: 'strict', STAKSMITH_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('disabled hooks take precedence over profile match', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: 'strict', STAKSMITH_DISABLED_HOOKS: 'my-hook' }, () => {
      assert.strictEqual(isHookEnabled('my-hook', { profiles: 'strict' }), false);
    });
  })) passed++; else failed++;
  ...
  if (test('allows standard profile by default', () => {
    withEnv({ STAKSMITH_HOOK_PROFILE: 'standard', STAKSMITH_DISABLED_HOOKS: undefined }, () => {
      assert.strictEqual(isHookEnabled('my-hook'), true);
    });
  })) passed++; else failed++;
