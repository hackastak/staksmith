...
For the vocabulary (deep module, interface, seam, depth), see the **`codebase-design`** skill — use its language throughout. That skill is the design-time half of this idea; this one is the mechanical enforcement of it.

**Scope: TypeScript, package-structured repos** — `src/packages/<name>/` or `packages/<name>/`. A single-package app has no boundaries for these rules to guard, so say so and stop rather than inventing a package layout the repo didn't ask for.
...
- **Package manager** — follow the Staksmith precedence: `CLAUDE_PACKAGE_MANAGER`, then `.claude/package-manager.json`, then `package.json`'s `packageManager` field, then lock-file detection (`pnpm-lock.yaml` → pnpm, `yarn.lock` → yarn, `bun.lockb` → bun, `package-lock.json` → npm), then the user's global preference, then npm. Use it for every command below (`pnpm`/`yarn`/`npm run`/`bunx`).
...
Then add a **context pointer** to it from the repo's agent-instructions file — **edit the one that exists**: `CLAUDE.md` if present, else `AGENTS.md`, creating `AGENTS.md` only if neither exists. Never create the second file alongside the first. One line is enough, e.g. `Packages are deep modules — see [src/packages/README.md](./src/packages/README.md) before adding or importing one.` This is what makes an agent discover the boundary rule instead of tripping over it.

This append has to coexist with the other skills that write to the same file — `setup-hackastak` (its `## Agent skills` block) and `vault-to-code-bridge`. Append your one line; don't reformat, reorder, or rewrite what's already there.
...
- **No auto-commit.** This skill installs a dependency, writes config, scaffolds a package, and edits `CLAUDE.md`/`AGENTS.md`. Stage all of it, summarise what changed, and hand back — the commit is the user's.

## Related skills

- **`codebase-design`** — the design-time vocabulary and principles this skill enforces mechanically.
- **`setup-pre-commit`** — the commit-time gate. Once both exist, fold `lint:boundaries` into the pre-commit hook.
- **`improve-codebase-architecture`** — finds the deepening opportunities; this keeps them from eroding afterwards.
