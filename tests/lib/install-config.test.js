...
function runTests() {
  ...
  if (test('resolves relative config paths from the provided cwd', () => {
    ...
    const resolved = resolveInstallConfigPath('configs/staksmith-install.json', { cwd });
    assert.strictEqual(resolved, path.join(cwd, 'configs', 'staksmith-install.json'));
  })) passed++; else failed++;
  ...
  if (test('loads and normalizes a valid install config', () => {
    ...
    try {
      const configPath = path.join(cwd, 'staksmith-install.json');
      ...
      const config = loadInstallConfig('staksmith-install.json', { cwd });
  ...
  if (test('rejects invalid config schema values', () => {
    ...
    try {
      writeJson(path.join(cwd, 'staksmith-install.json'), {
        version: 2,
        target: 'ghost-target',
      });

      assert.throws(
        () => loadInstallConfig('staksmith-install.json', { cwd }),
