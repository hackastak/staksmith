...
function maybeRunQualityGate(filePath) {
  ...
  const fix = String(process.env.STAKSMITH_QUALITY_GATE_FIX || '').toLowerCase() === 'true';
  const strict = String(process.env.STAKSMITH_QUALITY_GATE_STRICT || '').toLowerCase() === 'true';
