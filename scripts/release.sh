...
# Replace only the FIRST "version" occurrence in a file. BSD sed silently ignores
# GNU's 0,/re/ address form, so this uses awk to stay portable across macOS/Linux.
update_first_version() {
  local file="$1"
  local tmp="${file}.tmp"
  awk -v ver="$VERSION" '
    !replaced && /"version": *"[^"]*"/ {
